...
export default function PaymentSuccess() {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.big}>✅ Payment Success</Text>
      <Text style={styles.small}>Group: {groupId ?? "—"}</Text>

      <Pressable style={styles.btn} onPress={() => router.replace("/tabs/groups")}>
        <Text style={styles.btnText}>Go to My Groups</Text>
      </Pressable>

      {!!groupId && (
        <Pressable style={[styles.btn, styles.secondary]} onPress={() => router.replace(`/group/${groupId}`)}>
          <Text style={[styles.btnText, { color: "#111" }]}>Open Group</Text>
        </Pressable>
      )}
    </View>
  );
